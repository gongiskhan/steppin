package com.toptal.steppin.webapp;

import com.toptal.steppin.webapp.config.WebServerConfig;
import com.toptal.steppin.webapp.config.WebServerContext;
...
public class WebAppRunner {
    ...
    static Logger logger = LoggerFactory.getLogger(WebAppRunner.class);

    public static void main(String[] args) {
        try {
            new WebAppRunner().run(args);
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    ...
    private void run(String[] args) throws Exception {
        ...
        URL log4jUrl = WebAppRunner.class.getProtectionDomain().getClassLoader().getResource("log4j.properties");
        if(log4jUrl != null){
        ...
        options.addOption("jdbc_databaseurl", true, null);
